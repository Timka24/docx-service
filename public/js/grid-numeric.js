export function createNumericRow(cfg) {
  ...
  function getActiveValue() {
    const raw = (elInput()?.value ?? "").toString().trim();
    if (!raw) return "";
  ...
  function bindPaintToContainer() {
    ...
    grid.addEventListener("pointerdown", (e) => {
      ...
      if (anchor != null) {
        ...
        if (v === null) setHint("Ошибка: введите число (пример: 2 или 2,5)");
        else setHint(`Диапазон: ${anchor}… (мл: ${v || "Пусто"})`);
      }
    });
    ...
    grid.addEventListener("pointermove", (e) => {
      ...
      const v = getActiveValue();
      if (v === null) setHint("Ошибка: введите число (пример: 2 или 2,5)");
      else setHint(`Диапазон: ${Math.min(anchor, last)}–${Math.max(anchor, last)} (мл: ${v || "Пусто"})`);
    });
  ...
  function bindRangePreviewToContainer() {
    ...
    grid.addEventListener("pointermove", (e) => {
      ...
      const v = getActiveValue();
      if (v === null) setHint("Ошибка: введите число (пример: 2 или 2,5)");
      else setHint(`Предпросмотр: ${Math.min(rangeStart, last)}–${Math.max(rangeStart, last)} (мл: ${v || "Пусто"})`);
    });
  ...
  function init() {
    document.getElementById(cfg.clearValueBtnId)?.addEventListener("click", () => {
      const inp = elInput();
      if (inp) inp.value = "";
  ...
  return {
    ...
    setData(arr) {
      data = new Array(minutes).fill("").map((_, idx) => (arr?.[idx] ?? "").toString().trim());
      render();
    },
    getData() {
      return data.slice();
