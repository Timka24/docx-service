...
function renderSummary(archive) {
  var _archive$id;
  summaryArchiveId.textContent = String((_archive$id = archive.id) !== null && _archive$id !== void 0 ? _archive$id : "—");
...
function renderDocxControls(archive, renders) {
  const versions = renders.filter(row => typeof row.docx_key === "string" && row.docx_key.trim()).map(row => row.version);
...
function renderHistory(archive, renders) {
  ...
  renders.forEach(render => {
    ...
    const docxDownload = typeof render.docx_key === "string" && render.docx_key.trim() ? `<a class="btn btn-secondary" href="/api/archive/${archive.id}/download/docx?version=${render.version}">Скачать</a>` : '<span class="muted">—</span>';
    const pdfDownload = typeof render.pdf_key === "string" && render.pdf_key.trim() ? `<a class="btn btn-secondary" href="/api/archive/${archive.id}/download/pdf">Скачать</a>` : '<span class="muted">—</span>';
    rows.push(`
...
loadArchiveCard();
//# sourceMappingURL=archive-card.js.map