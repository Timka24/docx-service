...
async function createRenderVersion(pool, archiveId) {
  ...
  try {
    ...
    const pendingRender = await client.query(
      `select id, version
         from archive_renders
        where archive_id = $1
          and (docx_status = 'pending' or pdf_status = 'pending')
     order by version desc
        limit 1`,
      [normalizedArchiveId]
    );
    if (pendingRender.rows[0]) {
      await client.query("commit");
      return {
        renderId: pendingRender.rows[0].id,
        version: pendingRender.rows[0].version,
        alreadyPending: true,
      };
    }
    ...
    await client.query("commit");
    return { renderId: ins.rows[0].id, version: ins.rows[0].version, alreadyPending: false };
  } catch (e) {
    await client.query("rollback");
    throw e;
  } finally {
