const { parseStrictDate } = require("./date-utils");
...
const DATE_FIELDS = new Set(["pr_date", "pr_date_iso_raw", "end_date", "end_date_iso_raw"]);
...
function validateDateField(details, key, value) {
  ...
  if (!parseStrictDate(value)) {
    details.push({ path: key, message: "must be DD.MM.YYYY or YYYY-MM-DD calendar date" });
  }
}
...
function validateGridField(details, key, value) {
  ...
  for (let idx = 0; idx < value.length; idx += 1) {
    ...
    if (NUMERIC_GRID_FIELDS.has(key)) {
      if (element === "") continue;
...
function validateClockField(details, key, value) {
  if (value === "" || value === null || value === undefined) return;
  if (typeof value !== "string") {
    pushTypeError(details, key, "string");
    return;
  ...
  if (!/^(?:[01]?\d|2[0-3]):[0-5]\d$/.test(raw)) {
    details.push({ path: key, message: "must be HH:MM (00:00..23:59)" });
  }
}
...
function validatePayload(payload) {
  ...
  for (const [key, value] of Object.entries(payload)) {
    ...
    if (key === "i_t") {
      validateClockField(details, key, value);
      continue;
    }
    ...
    if (value.length > MAX_DEFAULT_TEXT) {
