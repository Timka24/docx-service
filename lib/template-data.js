...
function buildTemplateData(body) {
  ...
  const chDosageMl = normalizeMl70(ch_dosage_ml_marks);
  ...
  const chrono_dosage_ml = {};
  for (let i = 1; i <= MINUTES; i++){
    chrono_dosage_ml[`ch_dosage_ml_${pad2(i)}`] = chDosageMl[i - 1]
  }
