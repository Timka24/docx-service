...
Поле `details` добавляется, когда сервер может указать конкретные поля. UI формы показывает `message`, а для `validation_error` дополнительно выводит список деталей прямо в блоке статуса формирования.
